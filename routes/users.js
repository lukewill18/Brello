...
router.post('/register/', function(req, res, next) {  
  ...
  if(first_name.trim() == "" || last_name.trim() == "" || email.trim() == "" || password.trim() == "")
    next(createError(HTTPStatus.BAD_REQUEST, "One or more fields left blank"));
      ...
      sequelize.query("SELECT email FROM users WHERE email = '" + email.trim() + "'").then(function(response) {
        if(response[0].length > 0) {
          next(createError(HTTPStatus.BAD_REQUEST, "Account already exists with given email"));
        }
        else {
          sequelize.query(`INSERT INTO users VALUES (DEFAULT, '${first_name}', '${last_name}', '${password}', '${email}') RETURNING id`).then(function(response) {
...
router.post("/login/", function(req, res, next) {
  ...
  if(email.trim() == "" || password.trim() == "")
    ...
    sequelize.query("SELECT id FROM users WHERE email = '" + email.trim() + "' AND password = '" + password.trim() + "'").then(function(response)
    {
      if(response[0].length > 0) {
        req.session.id = response[0][0].id;
